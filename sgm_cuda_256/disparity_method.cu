...
Mat_<unsigned char, 1> compute_disparity_method(Mat_<unsigned char, 1> left, Mat_<unsigned char, 1> right, float *elapsed_time_ms, const char* directory, const char* fname) {
	...
	CUDA_CHECK_RETURN(cudaMemcpyAsync(d_im0, left.ptr(), sizeof(uint8_t)*size, cudaMemcpyHostToDevice, stream1));
	CUDA_CHECK_RETURN(cudaMemcpyAsync(d_im1, right.ptr(), sizeof(uint8_t)*size, cudaMemcpyHostToDevice, stream1));
	...
	Mat_<unsigned char, 1> disparity(rows, cols, h_disparity);
	return disparity;
}
